...
let __stixValidationErrorVerbose = false;
class StixValidationError extends Error {
    constructor(...args) {
        ...
        if (msg === undefined) {
            msg = "<< Unspecified STIX Validation Error >>";
        }

        if(Array.isArray(msg)) {
            if(err !== undefined) {
                throw new TypeError("StixValidationError: can't pass in error with an array of messages");
            }

            let newMsg = `${msg.length} STIX Validation Errors:\n`;
            let newErr = [];

            msg.forEach((m) => {
                if(m instanceof StixValidationError) {
                    newErr.push(m.details);
                    m = m.message;
                }
            ...
            if(newErr.length && newErr.length !== msg.length) {
                throw new TypeError("StixValidationError: expected message array to be all strings or all StixValidationErrors");
            }
        ...
        this.name = "StixValidationError";
        this.details = err;

        // console.log("New StixValidationError!", msg, ...args);
    }

    static isStixValidationError(e) {
        if(e instanceof StixValidationError) return true;

        return false;
    }

    static setVerbose(flag) {
        if(flag === undefined) return __stixValidationErrorVerbose;

        __stixValidationErrorVerbose = !!flag;
    }
}
...
class StixValidator {
    ...
    toFriendlyErrors() {
        this.errorList = this.errorList.map((e) => {
            return new StixValidationError(e, toFriendlyError(e));
...
function toFriendlyError(err) {
    ...
    case "oneOf":
    case "not":
        return `Object at path '${err.dataPath}' ${err.message}'. Value: ${toFriendlyDataType(err.data)}. ${schemaMsg}`;
...
function toFriendlyPattern(err) {
    ...
    case "^([a-zA-Z][a-zA-Z0-9+.-]*):(?:\\/\\/((?:(?=((?:[a-zA-Z0-9-._~!$&'()*+,;=:]|%[0-9a-fA-F]{2})*))(\\3)@)?(?=((?:\\[?(?:::[a-fA-F0-9]+(?::[a-fA-F0-9]+)?|(?:[a-fA-F0-9]+:)+(?::[a-fA-F0-9]+)+|(?:[a-fA-F0-9]+:)+(?::|(?:[a-fA-F0-9]+:?)*))\\]?)|(?:[a-zA-Z0-9-._~!$&'()*+,;=]|%[0-9a-fA-F]{2})*))\\5(?::(?=(\\d*))\\6)?)(\\/(?=((?:[a-zA-Z0-9-._~!$&'()*+,;=:@\\/]|%[0-9a-fA-F]{2})*))\\8)?|(\\/?(?!\\/)(?=((?:[a-zA-Z0-9-._~!$&'()*+,;=:@\\/]|%[0-9a-fA-F]{2})*))\\10)?)(?:\\?(?=((?:[a-zA-Z0-9-._~!$&'()*+,;=:@\\/?]|%[0-9a-fA-F]{2})*))\\11)?(?:#(?=((?:[a-zA-Z0-9-._~!$&'()*+,;=:@\\/?]|%[0-9a-fA-F]{2})*))\\12)?$":
        return `Property '${err.dataPath}' must be a RFC3986 compliant URL like "https://google.com/foo", value is: "${err.data}". ${schemaMsg}`;
    default:
        // console.log("PATTERN", err.params.pattern);
